...
#include "BMP280.h" //заголовочный файл
/*Объявления переменных и их типы*/
signed long temperature_raw, pressure_raw;// типы данных для темп. и давлен.(влажн.)
unsigned short dig_T1, dig_P1;// типы данных для темп.(цифр.), давл.(цифр)
...
/*Функция для считывания регистров по протоколу I2C */
uint8_t I2C_Read_Register(uint8_t device_adr, uint8_t internal_adr)
{
	...
	HAL_I2C_Master_Transmit(&hi2c1, device_adr, &tx_buff[0], 1, 10000); //HAL(см.библиотеку)
	HAL_I2C_Master_Receive(&hi2c1, device_adr + 1, &rx_buff[0], 1, 10000);


	return rx_buff[0];
}
/*Функция для записи регистров по протоколу I2C */
...
/*Функция для получения калиброванных значений*/
...
/*Значение получаемых переменных от BMP280 с оператором сдвига*/
...
{
	/*см.Datasheet BMP280*/
	I2C_Write_Register(BMP280_dev_address, 0xF4, register_F4);//osrs_t 010 x2, osrs_p 16 101, рабочий режим 11
...
volatile	double p=1048576.0-(double)pressure_raw;
	...
	temperature=T;
	pressure=p;
	altitude=44330.0f*(1-powf(pressure/101325.0f,1.0f/5.255f));//Выс.уров.мор.=((powf(101325.0/давл., 1/5.257f)-1)*(темп.+273.15f))/0.0065f;
}
